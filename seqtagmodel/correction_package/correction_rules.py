import seqtagmodel.correction_package.utils as utils


def correct_rule_1(sentence, tags, index):
    """Conjunction de/da must written separately. """
    word = utils.normalize(sentence[index])
...
def correct_rule_6(sentence, tags, index):
    ...
    apostrophe = "'" if sentence[index].replace(".", "").isdigit() or (
            index != 0 and utils.is_capitalized(sentence[index])) else ""
...
def correct_rule_9(sentence, tags, index):
    """Remove the vowel between two consonants in the beg"""
...
def correct_rule_12(sentence, tags, index):
    ...
    word = sentence[index]
    reverse_dict = utils.get_reverse_dictionaries(12)
...
def correct_rule_23(sentence, tags, index):
    ...
    sentence[index] = utils.capitalize(sentence[index])
    tags[index] = 0
...
def correct_with_dictionary(rule_name, adds_space=False):
    ...
    Returns: function
    """
    reverse_dict = utils.get_reverse_dictionaries(rule_name)

    def correction_function(sentence, tags, index):
        for wrong, correct in reverse_dict.items():
            if adds_space:
                current = utils.normalize(sentence[index]) + " " + utils.normalize(sentence[index + 1])
                if wrong in current:
                    new = current.replace(wrong, correct)
                    sentence[index] = utils.capitalize(new) if utils.is_capitalized(sentence[index]) else new
                    sentence[index + 1] = ""
                    tags[index] = 0
                    tags[index + 1] = 0
                    return
                ...
                current = utils.normalize(sentence[index])
                wrong = utils.normalize(wrong)
                if wrong in current:
                    new = current.replace(wrong, correct)
                    sentence[index] = utils.capitalize(new) if utils.is_capitalized(sentence[index]) else new
                    tags[index] = 0
