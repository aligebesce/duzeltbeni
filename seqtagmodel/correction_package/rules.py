import seqtagmodel.correction_package.dictionaries as dictionaries
from seqtagmodel.correction_package.utils import *
...
def rule_3(sentence, morp_analysis, flags, p, name):
    """Conjunction de/da never can be spelled as te/ta."""
...
def rule_4(sentence, morp_analysis, flags, p, name):
    """"da" should always written separately when it is used with a ya. (i.e. "ya da")"""

    tags = []
    if ("ya" in sentence) and ("da" in sentence) and ((sentence.index("da") - sentence.index("ya")) == 1) and flags[
        sentence.index("ya")] and flags[sentence.index("da")] and flip_coin(p):
...
def rule_5(sentence, morp_analysis, flags, p, name):
    ...
    for i in range(len(sentence)):
        ...
        if flags[i] and (word == "de" or word == "da") and i != 0 and flags[i - 1] and (
                not "'" in sentence[i - 1]) and flip_coin(p):
            tags.append(f"A {i - 1} {i}|||{name}|||{sentence[i - 1]} {sentence[i]}|||REQUIRED|||-NONE-|||0")
            sentence[i - 1] = sentence[i - 1] + "\'" + sentence[i]
            sentence[i] = ""
            flags[i] = False
...
def rule_6(sentence, morp_analysis, flags, p, name):
    ...
    for i in range(len(sentence)):
        ...
        if flags[i] and (not "\"" in sentence[i]) and \
                ...
                (not "'" in sentence[i] or (
                        i != 0 and is_capitalized(sentence[i]) and ("'de" in sentence[i] or "'da" in sentence[i]))) and \
...
# A
# S Oysa ki ne olacağını ona söylemiştim.

def rule_9(sentence, morp_analysis, flags, p, name):
    """Western-origin quotations that start with double consonants are written without a vowel between the consonants."""

    tags = []
    for i in range(len(sentence)):
        word = normalize(sentence[i])

        if flags[i] and len(word) >= 4 and (word[0] in consonants) and (word[1] in consonants) and (
                not is_capitalized(word.split("'")[0])) and flip_coin(p):
            tags.append(f"A {i} {i + 1}|||{name}|||{sentence[i]}|||REQUIRED|||-NONE-|||0")
...
def rule_11(sentence, morp_analysis, flags, p, name):
    ...
    for i in range(len(sentence)):
        ...
        if flags[i] and (word.startswith("mu") or word.startswith("mü") or word.startswith("mi") or word.startswith(
                "mı")) and i != 0 and flags[i - 1] and ("Ques" in morp_analysis()[i][3]) and flip_coin(p):
            tags.append(f"A {i - 1} {i}|||{name}|||{sentence[i - 1]} {sentence[i]}|||REQUIRED|||-NONE-|||0")
            sentence[i - 1] = sentence[i - 1] + sentence[i]
            sentence[i] = ""
            flags[i] = False
    ...
    for i in range(len(sentence)):
        ...
        if flags[i]:
            ...
            for key, value in mapping.items():
                ...
                if (key in word) and (word.index(key) > 3) and (not value in word) and (
                        "Verb" in morp_analysis()[i][3]) and flip_coin(p):
                    tags.append(f"A {i} {i + 1}|||{name}|||{sentence[i]}|||REQUIRED|||-NONE-|||0")
                    sentence[i] = sentence[i].replace(key, value)

                    # Note there is a weird case when there exists multiple suffixes starting with -a/-e.
                    # One could do the following errors
                    # yorumlayamayacağı -> correct
                    # yorumlıyamayacağı -> unnatural error
                    # yorumlayamıyacağı -> natural error
                    # yorumlıyamıyacağı -> natural error
                    # For the example above, we are focusing on the last error, where we assume user will be consistent with the errors he/she is making.
                    flags[i] = False
                    break
...
def rule_13(sentence, morp_analysis, flags, p, name):
    """Some words that has two syllables lose a vowel from the second syllable."""
...
def rule_17(sentence, morp_analysis, flags, p, name):
    """Compound words made with etmek, edilmek, eylemek, olmak, olunmak, are written seperatly unless there is a grammatic transformation."""
...
def rule_19(sentence, morp_analysis, flags, p, name):
    """Compound words are written jointly when second the meaning of the second changes."""
...
# Reference sample from CoNLL-2014 shared task.

# S Albinism is one of the examples .
# A 2 5|||ArtOrDet|||an|||REQUIRED|||-NONE-|||0
# A 5 6|||Nn|||example|||REQUIRED|||-NONE-|||0
